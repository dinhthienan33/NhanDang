...
def convert_to_onnx(model_name, ckpt_dir, ckpt_epoch, output_dir, input_shape=(1, 3, 512, 512), opset_version=11, use_mask=True):
    """
    Convert PyTorch model to ONNX format.
    
    Args:
        model_name: Model name (e.g., 'proposed')
        ...
        use_mask: Whether to export both generator and mask models
    """
    ...
    # Load model
    print(f"Loading model {model_name} with checkpoint epoch {ckpt_epoch}")
    model, _ = find_model(model_name, 'test')
    epoch = model.load(ckpt_dir, epoch=ckpt_epoch)
    ...
    # Export mask generator model
    if use_mask:
        mask_model_path = os.path.join(output_dir, f"{model_name}_mask_epoch{epoch}.onnx")
        print(f"Exporting mask generator to {mask_model_path}")
        
        # Export the mask generator model
        try:
            torch.onnx.export(
                model.generator_mask,
                dummy_input,
                mask_model_path,
                export_params=True,
            ...
            print("Mask generator model exported successfully")
        except Exception as e:
            print(f"Error exporting mask generator model: {e}")
    
    # Export main generator model
    generator_model_path = os.path.join(output_dir, f"{model_name}_generator_epoch{epoch}.onnx")
    print(f"Exporting main generator to {generator_model_path}")
    
    # For the generator, we need both the image and mask inputs
    mask_input = torch.randn((input_shape[0], 1, input_shape[2], input_shape[3]), device=device)
    combined_input = (torch.cat((dummy_input, mask_input), dim=1),)
    ...
    try:
        torch.onnx.export(
            ...
            output_names=['output', 'output_mid1', 'output_mid2'],
            dynamic_axes={
                'combined_input': {0: 'batch_size', 2: 'height', 3: 'width'},
                'output': {0: 'batch_size', 2: 'height', 3: 'width'},
                'output_mid1': {0: 'batch_size', 2: 'height', 3: 'width'},
                'output_mid2': {0: 'batch_size', 2: 'height', 3: 'width'}
            }
        )
        print("Generator model exported successfully")
    except Exception as e:
        print(f"Error exporting generator model: {e}")
    
    # Create a combined inference model for easy use
    print("Creating inference wrapper for ONNX models...")
    create_onnx_inference_wrapper(output_dir, model_name, epoch)
    
    print(f"ONNX conversion complete. Models saved to {output_dir}")
    return generator_model_path, mask_model_path if use_mask else None

def create_onnx_inference_wrapper(output_dir, model_name, epoch):
    """
    Create a Python wrapper for ONNX inference that matches the API of the original model
    """
    wrapper_path = os.path.join(output_dir, "onnx_inference.py")
...
'''
    ...
    print(f"Created ONNX inference wrapper at {wrapper_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert PyTorch rain removal model to ONNX format")
    parser.add_argument("--model", default="proposed", type=str, help="Model name")
    parser.add_argument("--ckpt_dir", default="backend/checkpoint", type=str, help="Directory containing model checkpoints")
    parser.add_argument("--ckpt_epoch", default=800, type=int, help="Checkpoint epoch to load")
    parser.add_argument("--output_dir", default="onnx_models", type=str, help="Output directory for ONNX model")
    parser.add_argument("--height", default=512, type=int, help="Input height for ONNX model")
    parser.add_argument("--width", default=512, type=int, help="Input width for ONNX model")
    parser.add_argument("--opset_version", default=11, type=int, help="ONNX opset version")
