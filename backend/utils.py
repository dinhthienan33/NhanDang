...
def load_weights(ckpt_dir, epoch=None):
    ...
    if len(ckpt_list) == 0:
        ...
        if epoch == None:
            ...
            # Use CPU device mapping for model loading
            model_weights = torch.load(os.path.join(ckpt_dir, ckpt_list[-1]), map_location=torch.device('cpu'))
        else:
            # Use CPU device mapping for model loading
            model_weights = torch.load(os.path.join(ckpt_dir, f'model_epoch{epoch}.pth'), map_location=torch.device('cpu'))
